...
interface Props {
  ...
  onSwapBidCaptain: (targetSeat: number) => void;
  onExtendGame: () => void;
  onFinishGame: () => void;
  // Spectator/peek mode
  isSpectator?: boolean;
...
export default function GameBoard({
  ...
  useEffect(() => {
    ...
    if (state.status !== "bidding") setShowBidSwap(false);
          ...
          <div className="hidden md:flex items-center gap-1.5">
            ...
            {iAmCaptain && state.teams_enabled && myTeam && myTeam.length > 1 && state.status === "playing" && (
              <div className="relative">
                <button
                  onClick={() => setShowBidSwap(v => !v)}
                  className={`text-[10px] border px-1.5 py-0.5 rounded transition-all ${
                    showBidSwap
                      ? "bg-yellow-400/15 border-yellow-400/50 text-yellow-300"
                      : "text-yellow-400/80 hover:text-yellow-300 border-yellow-400/25 hover:border-yellow-400/50"
                  }`}
                >
                  Pass bid {showBidSwap ? "▴" : "▾"}
                </button>
                {showBidSwap && (
                  <div className="absolute right-0 top-full mt-1 flex flex-col gap-1 bg-gray-900 border border-white/10 rounded-xl p-1.5 z-50 min-w-[100px]">
                    {myTeam.filter((seat) => seat !== me?.seat).map((seat) => {
                      const p = state.players.find((pl) => pl.seat === seat);
                      return p ? (
                        <button
                          key={seat}
                          onClick={() => { onSwapBidCaptain(seat); setShowBidSwap(false); }}
              ...
              <div className="w-full max-w-xs flex flex-col items-center gap-1.5 min-h-[40px]">
                {trickWinner ? (
                  ...
                  <>
                    ...
                    {/* Pass bid — shown to captain while their team hasn't bid yet */}
                    {iAmCaptain && state.teams_enabled && me && (me.bid < 0) && myTeam && (
                      <div className="mt-2">
                        {!showBidSwap ? (
                          <button
                            onClick={() => setShowBidSwap(true)}
                            className="w-full text-[11px] text-gray-500 hover:text-yellow-400 py-1 transition-colors"
                          >
                            Pass bid to teammate →
                          </button>
                        ) : (
                          <div className="flex flex-wrap gap-1.5 justify-center">
                            <span className="text-[11px] text-gray-500 w-full text-center mb-0.5">Pass bid to:</span>
                            {myTeam
                              .filter((seat) => seat !== me.seat)
                              .map((seat) => {
                                const p = state.players.find((pl) => pl.seat === seat);
                                return p ? (
                                  <button
